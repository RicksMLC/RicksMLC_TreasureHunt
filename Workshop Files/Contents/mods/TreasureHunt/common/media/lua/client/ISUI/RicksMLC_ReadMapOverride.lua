...
local origOnCheckMap = ISInventoryPaneContextMenu.onCheckMap
ISInventoryPaneContextMenu.onCheckMap = function(self, map, player)
    RicksMLC_MapIDLookup.Instance():SetReadingMap(self:getMapID(), self)
    origOnCheckMap(self, map, player)
end

local origOnClose = ISMapWrapper.close
ISMapWrapper.close = function(self)
    RicksMLC_MapIDLookup.Instance():SetReadingMap(nil, nil)
    origOnClose(self)
end
