...
function Concussion_Test:Run()
    ...
    local fwdDirVec = self.player:getForwardDirection()
    fwdDirVec:setX(-1)
    fwdDirVec:setY(0)
    self.player:setForwardDirection(fwdDirVec)
    self.player:setForceSprint(true)
    self.player:setSprinting(true)
    self.player:Move(fwdDirVec)
    DebugLog.log(DebugType.Mod, "Direction: x: " .. tostring(fwdDirVec:getX()) .. " y: " .. tostring(fwdDirVec:getY()))
